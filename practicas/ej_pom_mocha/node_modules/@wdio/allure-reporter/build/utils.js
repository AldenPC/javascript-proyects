...
exports.getTestStatus = (test, config) => {
    if (config.framework === 'jasmine') {
        return constants_1.testStatuses.FAILED;
    }
    if (test.error.name && test.error.message) {
        const message = test.error.message.trim();
        return (test.error.name === 'AssertionError' || message.includes('Expect')) ? constants_1.testStatuses.FAILED : constants_1.testStatuses.BROKEN;
    }
    if (test.error.name) {
        return test.error.name === 'AssertionError' ? constants_1.testStatuses.FAILED : constants_1.testStatuses.BROKEN;
    }
    if (test.error.stack) {
        const stackTrace = test.error.stack.trim();
        return (stackTrace.startsWith('AssertionError') || stackTrace.includes('Expect')) ? constants_1.testStatuses.FAILED : constants_1.testStatuses.BROKEN;
    }
    return constants_1.testStatuses.BROKEN;
};
